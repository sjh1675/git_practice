...
public class Admin_User_Info implements Initializable {
	...
	public void initialize(URL location, ResourceBundle resources) {
		...
		user_inpatient_in.setOnAction(e -> {
			List<UserVO> inList = user_tbl.getItems().stream().filter(vo -> vo.getChk().isSelected())
					.collect(Collectors.toList());
			for (UserVO u : inList) {
				int a = new Random().nextInt(5) + 5; // 층 (5~9층)
				int b = new Random().nextInt(9) + 1; // 호 (1~9호)
				int c = homeCont.helper.inpatientDAO.inpUser(new InpatientVO(u.getId(), u.getName(),
						Timestamp.valueOf(LocalDateTime.now()), a + "0" + b + "호"));
				if (c != 0) {
					Controllers.alt("입원 처리 완료", u.getName()+"님의 입원 처리가 완료되었습니다.", AlertType.INFORMATION);
				} else {
					Controllers.alt("입원 실패", u.getName()+"님은 이미 입원한 회원입니다.", AlertType.WARNING);
				}
				u.getChk().selectedProperty().set(false);
			}
		});
