...
public class User_Home implements Initializable {
	...
	@FXML
	private Button home_toMain, home_toDoctor, home_toResv, home_toChat, home_toRoute, home_toEdit, home_login_switch,
			home_exit;
	...
	public Pane getJoin() {	return join; }

	public Pane getLogin() { return login; }

	public Pane getMain() {	return main; }

	public Pane getViewWrap() { return viewWrap; }

	public UserVO getLoginUser() { return loginUser; }
	
	public void userLogout() { loginUser = null; }
	...
	private void setHandler() {
		...
		home_toEdit.setOnAction(e -> {
			if (loginUser != null)
				switchView(pwChk);
		});
		home_exit.setOnAction(e -> Platform.exit());
		home_toResv.setOnAction(e -> {
			if (loginUser == null) {
				switchView(login);
			} else {
				switchView(reserv);
